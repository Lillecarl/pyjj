...
* `<COMMAND>` — Command to run to determine the status of a revision
...
* `-r`, `--range <REVSETS>` — Range of revisions to bisect (can be repeated)
...
**Usage:** `jj bookmark untrack [OPTIONS] <BOOKMARK[@REMOTE]>...`

###### **Arguments:**

* `<BOOKMARK[@REMOTE]>` — Bookmark name patterns or remote bookmark symbols to untrack
...
* `-o`, `--option <OPTION>` — Send a `git push -o` option (can be repeated)
...
* `-r`, `--revision <REVSETS>` — Rebase the given revisions, rebasing descendants onto this revision's parent(s)
   ...
   If none of `-b`, `-s`, or `-r` is provided, then the default is `-b @`.
* `-o`, `--onto <REVSETS>` [alias: `destination`] — The revision(s) to rebase onto (can be repeated to create a merge commit)
* `-A`, `--insert-after <REVSETS>` [alias: `after`] — The revision(s) to insert after (can be repeated to create a merge commit)
* `-B`, `--insert-before <REVSETS>` [alias: `before`] — The revision(s) to insert before (can be repeated to create a merge commit)
...
**Usage:** `jj run [OPTIONS] <COMMAND> [ARGS]...`

###### **Arguments:**

* `<COMMAND>` — Command to run across all selected revisions
* `<ARGS>` — Arguments to pass to the command

   Hint: Use a `--` separator to allow passing arguments starting with `-`. For example `jj run --revisions=... -- cargo build --release`.
