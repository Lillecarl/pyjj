    ...
    try:
        ...
        if getattr(args, "revisions", None) is not None:
            ...
            if len(revs) == 1:
                ...
                if c.parent_ids:
                    ...
                    entries = [pyjj.DiffEntry(path=p, status="added", executable=False) for p in c.list_files(paths)]
                    # Fall back to simple listing if DiffEntry construction differs; use list_files
            ...
            if first.parent_ids:
                ...
                entries = [pyjj.DiffEntry(path=p, status="added", executable=False) for p in last.list_files(paths)]
            for e in entries:
                print(f"{e.status:8} {e.path}")
            return 0
        ...
        if from_rev is not None or to_rev is not None:
            ...
            if wc.parent_ids:
                ...
                entries = [pyjj.DiffEntry(path=p, status="added", executable=False) for p in wc.list_files(paths)]
        name_only = getattr(args, "name_only", False)
        for e in entries:
            if name_only:
                print(e.path)
